...
const ProductCard: React.FC<ProductCardProps> = ({data}) => {
  ...
  return (
    <div onClick={onClick} className='group cursor-pointer relative bg-white p-3 border border-light-grey rounded-3xl shadow-sm transition hover:shadow-lg space-y-4 min-w-[200px]'>
    ...
    (
      ...
      <div className="flex gap-x-6 rounded-full text-white items-center justify-center border border-transparent bg-orange h-5 w-5 p-3 hover:border-orange hover:text-orange hover:bg-transparent">
              
          <button onClick={decreaseQuantity}><Minus size={15}/></button>
      </div>
      <span className='text-black rounded-full flex items-center justify-center text-sm border text-center border-orange h-5 w-5 p-3'>
          {item.orderQuantity}
      </span>
      <div className="flex gap-x-6 rounded-full items-center text-white justify-center border border-transparent bg-orange h-5 w-5 p-3  hover:border-orange hover:text-orange hover:bg-transparent">
